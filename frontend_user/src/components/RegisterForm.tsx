...
export default function RegisterForm({ onSuccess }: RegisterProps) {
  ...
  const handleChange = (e: any) => {
    ...
    if (e.target.name === 'email') {
        ...
        setOtpSent(false);
        setOtpInput('');
        setShowRegisteredError(false); // Reset custom error
        setError(null);
    }
  };

  const handleEmailBlur = async () => {
    // Only check if not already verifying/verified to avoid spamming
    if (!formData.email || !formData.email.includes('@') || otpSent || isVerified) return;
  ...
  const RegisteredErrorBox = () => (
    <div style={{
        ...
        <p style={{ fontSize: '0.9rem', lineHeight: '1.5', color: 'rgba(255,255,255,0.9)' }}>
            This email is already registered. Only one free ticket is allowed per person.
        </p>
  ...
  const handleSendOTP = async () => {
    ...
    // Name is optional for verification now (defaults to 'Attendee')
    
    setLoading(true);
    setError(null);
    try {
        // Pre-check email to avoid 409 console error (User Request)
        ...
        // Fallback catch (though pre-check should catch duplicates)
        if (err.response?.status === 409 || err.response?.data?.message === 'This email is already registered.') {
             setShowRegisteredError(true);
             setError(null);
        } else {
    ...
    <div className="auth-page-container" style={{ padding: '0', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div className="glass-card" style={{ padding: '1.5rem 2rem', maxWidth: '550px', margin: '0', transform: 'scale(1)' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          ...
          <div className="input-group" style={{ marginBottom: 0 }}>
            <label className="input-label" style={{ marginBottom: '0.2rem', fontSize: '0.8rem' }}>Email Address<RequiredStar /></label>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                <input 
                  ...
                  style={{ width: '100%', borderColor: isVerified ? '#4ade80' : undefined, padding: '8px 10px', fontSize: '0.9rem' }}
                  readOnly={isVerified}
                />
                ...
                {!isVerified && !otpSent && (
                    <button 
                        ...
                        style={{ width: '100%', padding: '0.4rem', fontSize: '0.8rem', marginTop: '0.2rem' }}
            ...
            {otpSent && !isVerified && (
                  <div style={{ marginTop: '0.25rem', display: 'flex', flexDirection: 'column', gap: '0.25rem', animation: 'fadeIn 0.3s' }}>
                      ...
                      <button 
                        ...
                        className="btn-primary"
                        style={{ background: '#10b981', padding: '0.4rem', width: '100%', fontSize: '0.8rem' }}
                        disabled={loading}
                      >
                        CONFIRM OTP
                      </button>
          ...
          <div className="input-group" style={{ marginBottom: 0 }}>
             <label className="input-label" style={{ marginBottom: '0.2rem', fontSize: '0.8rem' }}>Phone Number<RequiredStar /></label>
             <input 
               className="form-input" 
               type="tel" 
               name="phone"
          ...
          <button type="submit" className="btn-primary" disabled={loading || showRegisteredError || (!!formData.email && !isVerified)} style={{ marginTop: '0.8rem', padding: '10px', fontSize: '1rem' }}>
